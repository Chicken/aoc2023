const cardTypes = "A, K, Q, J, T, 9, 8, 7, 6, 5, 4, 3, 2".split(", ").reverse();
    ...
    .map((l) => {
        ...
        for (const [r, a, b] of [[6, 5, 0], [5, 4, 0], [4, 3, 2], [3, 3, 0], [2, 2, 2], [1, 2, 0], [0, 1, 0]]) {
            ...
            for (const cardType of cardTypes) {
                if (hand.split("").filter((c) => c === cardType).length === a) {
                    containsA = true;
                    aCardType = cardType;
                    break;
                }
            ...
            if (b !== 0) {
                for (const cardType of cardTypes) {
                    if (hand.split("").filter((c) => c === cardType && c !== aCardType).length === b) {
                        containsB = true;
                        break;
                    }
                }
